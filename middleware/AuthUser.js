...
export const kasirOnly = async (req, res, next) =>{
    ...
    if(!user) return res.status(404).json({msg: "User tidak ditemukan"});
    if(user.role !== "kasir") return res.status(403).json({msg: "Akses terlarang, Anda bukan Kasir"});
    next();
}
...
export const manajerOnly = async (req, res, next) =>{
    ...
    if(!user) return res.status(404).json({msg: "User tidak ditemukan"});
    if(user.role !== "manajer") return res.status(403).json({msg: "Akses terlarang, Anda bukan Admin"});
    next();
}